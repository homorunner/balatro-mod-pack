...
--- MOD_AUTHOR: [Betmma, nicholassam6425, homorunner]
...
function addBooster(id, name, name_i18n, order, discovered, weight, kind, cost, pos, config, desc_i18n, alerted,
                    sprite_path, sprite_name, selection_state, color)
    ...
    local newBooster = {
        ...
        config = config,
        alerted = alerted,
        modded_sprite = sprite_path and sprite_name,
        key = id,
        modded = true,
                ...
                G.E_MANAGER:add_event(Event({
                    ...
                    func = function()
                        ...
                        for i = 1, _size do
                            ...
                            G.GAME.voucher_pack_name = self.ability
                            .name                              -- this is to tell get_next_voucher_key_ref in Betmma_Vouchers to change how to get pool
                            local voucher_key = get_next_voucher_key(true)
                            G.GAME.voucher_pack_name = nil
                            local card = Card(G.play.T.x + G.play.T.w / 2 - G.CARD_W * 1.27 / 2,
                                G.play.T.y + G.play.T.h / 2 - G.CARD_H * 1.27 / 2, G.CARD_W, G.CARD_H, G.P_CARDS.empty,
                                G.P_CENTERS[voucher_key], { bypass_discovery_center = true, bypass_discovery_ui = true })
        ...
        if _c.name and _c.name:find('Voucher Pack') then
            ...
            if first_pass and not (_c.set == 'Edition') and badges then
                for k, v in ipairs(badges) do
                    ...
                    if v == 'negative_consumable' then info_queue[#info_queue + 1] = { key = 'e_negative_consumable', set =
                        'Edition', config = { extra = 1 } } end
